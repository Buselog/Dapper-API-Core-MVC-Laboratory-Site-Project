using LaboraTechDapperCoreMVCLayer.Models;
using Microsoft.AspNetCore.Mvc;
using Newtonsoft.Json;

namespace LaboraTechDapperCoreMVCLayer.ViewComponents
{
    public class TestimonialViewComponentPartial : ViewComponent
    {
        public async Task<IViewComponentResult> InvokeAsync()
        {
            HttpClient client = new HttpClient(); // url formatında veri almak
            var response = await client.GetAsync("https://localhost:7102/api/Testimonial/TestimonialList");
            List<Testimonial> testimonialList = JsonConvert.DeserializeObject<List<Testimonial>>(await response.Content.ReadAsStringAsync());
            return View(testimonialList);
            
        }
    }
}
